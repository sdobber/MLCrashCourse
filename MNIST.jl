# This is a patched version of the latest MNIST.jl file (downloaded on Mar 6, 2019).
# Please see the original contribution at https://github.com/johnmyleswhite/MNIST.jl
...
module MNIST
    ...
    function imageheader(@compat(filename::AbstractString))
        io = open(filename, "r")
        magic_number = bswap(read!(io, Array{@compat(UInt32)}))
        total_items = bswap(read!(io, Array{@compat(UInt32)}))
        nrows = bswap(read!(io, Array{@compat(UInt32)}))
        ncols = bswap(read!(io, Array{@compat(UInt32)}))
    ...
    function getimage(@compat(filename::AbstractString), index::Integer)
        ...
        image_t = read!(io, Array{@compat(UInt8)}(undef, MNIST.NROWS, MNIST.NCOLS))
    ...
    function getlabel(@compat(filename::AbstractString), index::Integer)
        ...
        label = read!(io, Array{@compat(UInt8)}(undef,1))
    ...
    function trainlabel(index::Integer)
        convert(Float64, getlabel(TRAINLABELS, index)[1])
    end

    function testlabel(index::Integer)
        convert(Float64, getlabel(TESTLABELS, index)[1])
    end
